...
_params = ParamGrid([
    ('seed', [0000, 3333]),
    ('quads_obstacle_num', [2]),
    ('quads_obst_collision_enable_grace_period', [True]),
    ('quads_obst_level_change_cond', [1.0, 2.0]),
])

_experiment = Experiment(
    'test_inf_height_search_num_obst_level_cond_mix',
    QUAD_OBSTACLE_PARAMETERZE_CLI,
    _params.generate_params(randomize=False),
)

RUN_DESCRIPTION = RunDescription('test_quads_multi_obst_mix_8a_v116', experiments=[_experiment])
