...
_params = ParamGrid([
    ...
    ('reward_scale', [0.05]),
    ('quads_neighbor_proximity_mode', [0]),
    ('quads_collision_obst_smooth_max_penalty', [2, 4]),
    ('quads_collision_smooth_max_penalty', [0.1, 4.0]),
])

_experiment = Experiment(
    'v2_obst_prox_8_obst_2_local_obst_prox_penalty_mix',
    QUAD_OBSTACLE_PARAMETERZE_LONG_DURATION_CLI,
    _params.generate_params(randomize=False),
)

RUN_DESCRIPTION = RunDescription('v2_8_obst_2_local_quads_multi_obst_mix_8a_v116', experiments=[_experiment])
