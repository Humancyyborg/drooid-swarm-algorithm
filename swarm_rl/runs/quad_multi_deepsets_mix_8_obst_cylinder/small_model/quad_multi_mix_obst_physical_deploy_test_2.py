...
_params = ParamGrid([
    ('seed', seeds(2)),
    ('quads_early_termination', [False]),
    ('quads_curriculum_min_obst', [0, 2]),
    ('quads_pos_metric', ['normal', 'piecewise']),
    ('quads_crash_floor_broken_mode', [False]),
])
...
SMALL_MODEL_CLI = QUAD_8_OBSTACLES_PARAMETERZE_CLI + (
    ...
    '--quads_spawn_height_mode=1 --quads_broken_mode=False --quads_obstacle_num=10 '
    '--quads_obstacle_size=0.6 --quads_init_random_state=False'
)

_experiment = Experiment(
    'test-2-physical-deploy-curri-16-hidden-cylinder-small_model',
    SMALL_MODEL_CLI,
