...
def make_quadrotor_env_multi(cfg, render_mode=None, **kwargs):
    ...
    env = QuadrotorEnvMulti(
        ...
        # Obstacle
        use_obstacles=cfg.quads_use_obstacles, obst_obs_type=cfg.quads_obstacle_obs_type,
        obst_density=cfg.quads_obst_density, obst_size=cfg.quads_obst_size, obst_spawn_area=cfg.quads_obst_spawn_area,
        # Aerodynamics
        ...
        if cfg.quads_num_agents > 1:
            ...
            annealing += [
                ...
                AnnealSchedule('quadcol_bin_smooth_max',
                               cfg.quads_collision_smooth_max_penalty, cfg.anneal_collision_steps),
            ]
        if cfg.quads_use_obstacles:
            reward_shaping['quad_rewards']['quadcol_bin_obst'] = 0.0
            annealing.append(AnnealSchedule('quadcol_bin_obst',
                                            cfg.quads_obst_collision_reward, cfg.anneal_collision_steps))
    else:
        annealing = None
