...
def state_xyz_vxyz_R_omega_dfloor(self):
    if self.use_numba:
        pos, vel, rot, omega, acc = self.sense_noise.add_noise_numba(
            self.dynamics.pos,
            self.dynamics.vel,
    ...
    # return np.concatenate([pos[2] - 0.0])
    dist_floor = np.clip(pos[2] - self.dynamics.arm, a_min=self.room_box[0][2], a_max=self.room_box[1][2])
    dist_floor = np.array([dist_floor])
    return np.concatenate([pos - self.goal[:3], vel, rot.flatten(), omega, dist_floor])
