...
def state_xyz_vxyz_R_omega_wall(self):
    ...
    # return np.concatenate([pos - self.goal[:3], vel, rot.flatten(), omega, (pos[2],)])
    wall_1 = np.clip(pos - self.room_box[0], a_min=0.0, a_max=5.0)
    wall_2 = np.clip(self.room_box[1] - pos, a_min=0.0, a_max=5.0)
    return np.concatenate([pos - self.goal[:3], vel, rot.flatten(), omega, wall_1, wall_2])
