...
class QuadrotorSingle:
    ...
    def make_observation_space(self):
        ...
        self.obs_space_low_high = {
            ...
            "dist": [np.zeros(1), np.linalg.norm(self.room_box[1] - self.room_box[0]) * np.ones(1)]
        }
        ...
        elif self.swarm_obs == 'pos_vel_dist' and self.num_agents > 1:
            obs_comps = obs_comps + (['rxyz'] + ['rvxyz'] + ['dist']) * self.num_use_neighbor_obs
        elif self.swarm_obs == 'pos_vel_goals_dist' and self.num_agents > 1:
            obs_comps = obs_comps + (['rxyz'] + ['rvxyz'] + ['goal'] + ['dist']) * self.num_use_neighbor_obs
