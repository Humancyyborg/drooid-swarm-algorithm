...
class QuadrotorEnvMulti(gym.Env):
    def __init__(self, num_agents, ep_time, rew_coeff, obs_repr,
                 ...
                 # Obstacle
                 use_obstacles, obst_obs_type, obst_density, obst_size, obst_spawn_area,
        ...
        for i in range(self.num_agents):
            e = QuadrotorSingle(
                ...
                rew_coeff=rew_coeff, use_numba=use_numba,
                # Neighbor
                num_agents=num_agents,
                neighbor_obs_type=neighbor_obs_type, num_use_neighbor_obs=self.num_use_neighbor_obs,
                # Obstacle
                use_obstacles=use_obstacles, obst_obs_type=obst_obs_type,
                # Rendering info
                view_mode=quads_view_mode
            )
            self.envs.append(e)
        ...
        self.rew_coeff = dict(
            ...
            quadcol_bin=5., quadcol_bin_smooth_max=10., quadcol_bin_obst=5., quadcol_obst_coeff=1.
        )
    ...
    def step(self, actions):
        ...
        # # Update goals
        scenario_goals = np.array(self.scenario.goals)
        self.scenario.step()
        if np.array_equal(a1=scenario_goals, a2=np.array(self.scenario.goals)):
            goal_change_flag = False
        else:
            goal_change_flag = True

        # 5. Get final observations

        # If self state updates or goal changes
        if self_state_update_flag or goal_change_flag:
            # Collect velocities & omega after physical interaction
            for i in range(self.num_agents):
        ...
        # # TODO: Should use pos and vel in sensor noise to calculate neighbor obs
        if self.num_use_neighbor_obs > 0:
            obs = self.add_neighborhood_obs(obs)
